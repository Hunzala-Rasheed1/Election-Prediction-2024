...
# ==============================================================================
def predict_gallup(constituency_data, survey_name="Gallup_2024_1.csv"):
...
def predict_dunya(constituency_data):
    df_dunya_survey = pd.read_csv("data\\Gallup_Surveys\\Dunya_2024_1.csv")
    # constituency data
    constituency_number = constituency_data["Constituency Number (ID)"].iloc[0]
    constituency_number = str(constituency_number).split(" ")[0]
    list_parties = constituency_data["Party Affiliation"].tolist()
    ...
    if constituency_number in df_dunya_survey["Constituency"].tolist():
        ...
        for party in list_parties:
            try:
                ...
                party_popularity = df_dunya_survey[df_dunya_survey["Constituency"] == constituency_number][
                    party].tolist()
        ...
        candidate_prob = 0.1 * np.array(predict_gallup(constituency_data, survey_name="Gallup_2024_2.csv"))
...
# ==============================================================================
def predict_districtHistory(constituency_data, file_name="results_2013.csv"):
    ...
    if district_name in df_district_histroy["District"].tolist():
        ...
        try:
            for party in list_parties:
                ...
                if (party in history_parties):
                    # party populairty in the constituency
                    party_popularity = df_district_histroy[df_district_histroy["District"] == district_name][
                        party].tolist()
        ...
        candidate_prob = predict_gallup(constituency_data, survey_name="Gallup_2024_2.csv")
    return candidate_prob
...
def predict_twitter(constituency_data):
    ...
    with open("data\\twitter\\pakistan.json") as json_data:
